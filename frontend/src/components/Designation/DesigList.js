...
const DesignationList = ({ onEditHandler }) => {
  ...
  useEffect(() => {
    if (!Loading && pageData) {
      ...
      setDesignation(pageData?.data);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [Loading, pageData]);

  const path = `/designation?page=${page.currentPage}&perPage=${rowsPerPage}`;
