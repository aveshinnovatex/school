...
import { updateDataWithFile, httpActions } from "../../redux/http-slice";
...
const EditStudentForm = ({ stuData, feeRecordData }) => {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const { updatedData: response } = useSelector((state) => state.httpRequest);
  ...
  //   const standardData = await getStandard();
  //   const stateData = await getState();
  //   const courseTypeData = await getCourseTyppe();
  //   const CastCategoryData = await getCastCategory();
  //   const studentCategoryData = await getStudentCategory();
  //   const feeDiscountData = await getFeeDiscount();

  //   const allData = feeDiscountData?.map((item) => ({
    ...
    const fetchData = async () => {
      try {
        const [
          ...
          instance.get(`/city/all`),
          instance.get(`/locality/all`),
          instance.get(`/state/all`),
          instance.get(`/course-type/all`),
          instance.get(`/cast-category/all`),
          instance.get(`/student-category/all`),
          instance.get(`/fee-head/all`),
        ]);

        setData((prevState) => ({
          ...prevState,
          city: cityResp?.data?.data,
          locality: localityResp?.data?.data,
          state: stateResp?.data?.data,
          courseType: courseTypeResp?.data?.data,
          castCategory: castCategoryResp?.data?.data,
          studentCategory: stuCategoryResp?.data?.data,
          vehiclesRoutes: routsResp.data.data,
          feeHeads: feeHeadResp.data.data,
        }));
      } catch (error) {
        if (
          error?.response?.data?.status === 401 ||
  ...
  //       const driverData = await instance.get(
  //         "/vehicle-driver/all?search=" + JSON.stringify({ vehicle: vehicleId })
  //       );
  ...
  // useEffect(() => {
  //   if (selectedData?.stoppage) {
  //     setTransportFee(selectedData?.stoppage?.transportFee);
  //   } else {
  //     setTransportFee(0);
  //   }
  ...
  useEffect(() => {
    const fetchFeeStructureData = async (searchData) => {
      try {
    ...
    if ((selectedData?.standard, selectedData?.section)) {
      ...
      fetchFeeStructureData(searchQuery);
    }
  ...
  useEffect(() => {
    ...
    if (
      ...
      (stuData?.standard?._id !== response?.data?.standard ||
        stuData?.section?._id !== response?.data?.section)
    ) {
      ...
      console.log("stuData?.standard", stuData?.standard?._id);
      console.log("response?.data?.standard", response?.data?.standard);

      console.log("stuData?.section", stuData?.section?._id);
      console.log("response?.data?.standard", response?.data?.section);
  ...
  }, [selectedData?.standard, stuData?.section]);
  ...
  useEffect(() => {
    if (stuData) {
      setValue("admissionNo", stuData?.admissionNo);
      setValue("registrationNo", stuData?.registrationNo);
      setValue("srnNo", stuData?.srnNo);
      setValue("firstName", stuData?.firstName);
      setValue("middleName", stuData?.middleName);
      setValue("lastName", stuData?.lastName);
      setValue("stuMobileNo", stuData?.stuMobileNo);
      setValue("stuAadharNo", stuData?.stuAadharNo);
      setValue("email", stuData?.email);
      setValue("rollNo", stuData?.rollNo);
      setValue("CBSERegistrationNumber", stuData?.CBSERegistrationNumber);
      setValue("EnrollmentNo", stuData?.EnrollmentNo);
      setValue("TransferCertificateNo", stuData?.TransferCertificateNo);
      setValue("religion", stuData?.religion);
      setValue("bloodGroup", stuData?.bloodGroup);
      setValue("motherTongue", stuData?.motherTongue);
      setValue("identificationMarks", stuData?.identificationMarks);
      setValue("fatherName", stuData?.fatherName);
      setValue("fatherAadharNo", stuData?.fatherAadharNo);
      setValue("motherAadharNo", stuData?.motherAadharNo);
      setValue("motherName", stuData?.motherName);
      setValue("fatherQualification", stuData?.fatherQualification);
      setValue("fatherOccupation", stuData?.fatherOccupation);
      setValue("parentMobileNo", stuData?.parentMobileNo);
      setValue("whatsappMobileNo", stuData?.whatsappMobileNo);
      setValue("bankAccountNumber", stuData?.bankAccountNumber);
      setValue("annualIncome", stuData?.annualIncome);
      setValue("relation", stuData?.relation);
      setValue("correspondenceAdd", stuData?.correspondenceAdd);
      setValue("permanentAdd", stuData?.permanentAdd);
      setValue("postOffice", stuData?.postOffice);
      setValue("district", stuData?.district);
      setValue("pinCode", stuData?.pinCode);
      setValue("nationality", stuData?.nationality);
      setValue("schoolName", stuData?.schoolName);
      setValue("board", stuData?.board);
      setValue("previousStandard", stuData?.previousStandard);
      setValue("previousSchoolSub", stuData?.previousSchoolSub);
      setValue("TransferCertificateNo", stuData?.TransferCertificateNo);
      setValue("password", stuData?.password);

      setDate((prevState) => ({
        ...prevState,
        dateOfBirth: stuData?.dateOfBirth,
        passingYear: stuData?.passingYear,
        admissionDate: stuData?.admissionDate,
        registrationDate: stuData?.registrationDate,
        joiningDate: stuData?.joiningDate,
        fatherDateOfBirth: stuData?.fatherDateOfBirth,
      }));

      setEditedValue((prevState) => ({
        ...prevState,
        id: stuData?._id,
        totalMarks: stuData?.totalMarks,
        obtainedMarks: stuData?.obtainedMarks,
        percentageCGPA: stuData?.percentageCGPA,
        passingYear: stuData?.passingYear,
        dateOfBirth: stuData?.dateOfBirth,
        registrationDate: stuData?.registrationDate,
        joiningDate: stuData?.joiningDate,
        fatherDateOfBirth: stuData?.fatherDateOfBirth,
        photo: stuData?.photo,
        marksheet: stuData?.marksheet,
        aadharCard: stuData?.aadharCard,
      }));

      const seletedGender = gender.find((item) => {
        return item?.value === stuData?.gender;
      });

      const selectedSalutation = salutation.find((item) => {
        return item?.value === stuData?.salutation;
      });

      const selectedPaper = modifiedArray(stuData?.paper, "paper");
      const selectedAdditionalPaper = modifiedArray(
        stuData?.additionalPaper,
        "paper"
      );

      const allData = stuData?.feeDiscount?.map((item) => ({
        _id: item?.feeDiscountId?._id,
        discountName: item?.feeDiscountId?.discountName,
        discountMode: item?.feeDiscountId?.discountMode,
        discountValue: item?.feeDiscountId?.discountValue,
        feeHeadDetails: {
          _id: item?.feeHeadId?._id,
      ...
      setSelectedData((prevState) => ({
        ...prevState,
        session: stuData?.session,
        standard: stuData?.standard,
        section: stuData?.section,
        enqPurpose: stuData?.enquiryPurpose,
        courseType: stuData?.courseType,
        city: stuData?.city,
        locality: stuData?.locality,
        state: stuData?.state,
        salutation: selectedSalutation,
        gender: seletedGender,
        cast: stuData?.cast,
        castCategory: stuData?.castCategory,
        studentCategory: stuData?.studentCategory,
        isStudent: stuData?.isStudent,
        isHostelerStudent: stuData?.isHostelerStudent,
        paper: selectedPaper,
        feeDiscount: allData || [],
        additionalPaper: selectedAdditionalPaper,
      }));
    }
  }, [stuData, setValue]);
  ...
  useEffect(() => {
    if (stuData) {
      if (
        stuData?.standard?._id === selectedData?.standard?._id &&
        stuData?.section?._id === selectedData?.section?._id &&
        stuData?.session?._id === selectedData?.session?._id
      ) {
        setDisable(true);
      } else {
        setDisable(false);
  ...
  useEffect(() => {
    if (feeRecordData.length > 0) {
      const openingBalanceData = feeRecordData?.filter(
        (feeData) => feeData?.isPreviousBalance === true
      );
      const previousData = openingBalanceData.map((feeData) => ({
        name: {
          name: feeData.feeHead,
          _id: feeData.feeHead._id,
          paidMonth: ["April"],
                  ...
                  >
                    <DemoItem>
                      <DatePicker
                        ...
                        defaultValue={dayjs(date?.admissionDate || null)}
                        label="Admission date"
                      />
                    </DemoItem>
                  </DemoContainer>
                  ...
                  >
                    <DemoItem>
                      <DatePicker
                        ...
                        defaultValue={dayjs(date?.registrationDate || null)}
                        label="Registration date"
                      />
                    </DemoItem>
                  </DemoContainer>
          ...
          <Grid container spacing={2} wrap="wrap">
            <Grid item md={4} sm={6} xs={12}>
              ...
              <PhotoUpload
                ...
                existingFile={
                  editValue?.photo ? `Student/photo/${editValue.photo}` : ""
                }
                field={photoField}
            ...
            <Grid item md={4} sm={6} xs={12}>
              ...
              <Upload
                ...
                existingFile={
                  editValue?.marksheet
                    ? `Student/marksheet/${editValue.marksheet}`
                    : ""
                }
              ...
              <Upload
                ...
                existingFile={
                  editValue?.aadharCard
                    ? `Student/aadhar/${editValue.aadharCard}`
                    : ""
                }
                field={aadharField}
                fileAccept=".pdf, .jpg, .jpeg, .png"
