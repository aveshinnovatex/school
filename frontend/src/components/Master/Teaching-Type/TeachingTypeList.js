...
const TeachingTypeList = ({ onEditHandler }) => {
  const [page, setPage] = useState({ currentPage: 0, totaltems: 0 });
  const [rowsPerPage, setRowsPerPage] = React.useState(10);
  const [index, setIndex] = useState(1);
  const [teacingType, setTeacingType] = useState([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [teacingTypeIdToDelete, setTeacingTypeIdToDelete] = useState(null);
  ...
  useEffect(() => {
    if (!Loading && pageData) {
      ...
      setTeacingType(pageData?.data);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [Loading, pageData]);

  const path = `/teaching-type?page=${page.currentPage}&perPage=${rowsPerPage}`;

  useEffect(() => {
    const fetchteacingType = async () => {
      try {
  ...
  const filterList = (index) => {
    const updatedteacingType = [...teacingType];
    updatedteacingType.splice(index, 1);
    setTeacingType(updatedteacingType);
  };

  // delete account group
  const deleteHandler = async (id) => {
    setIsModalOpen(true);
    setTeacingTypeIdToDelete(id);
  };
      ...
      {!Loading ? (
        <TableContainer>
          <Table sx={{ minWidth: 250 }} size="small">
            ...
            <TableBody>
              {teacingType && teacingType.length > 0 ? (
                teacingType.map((row, indx) => (
                  <TableRow
                    hover
                    key={row?._id}
...
export default TeachingTypeList;
